...
void main() {
	...
	float dblend = sin(rand(vec2(g_Time*55,g_Time*55)));
	dblend = sign(dblend) * pow(abs(max(0.00001, dblend)), 4.0);
	vec2 distortion = vec2(dblend *
							g_DistortionStrength * 0.3 *
								(0.01 * g_DistortionWidth*0.0001,  abs(frac(g_Time * g_DistortionSpeed) - v_TexCoord.y/v_TexCoord.x*v_TexCoord.x)),
							0.0);
