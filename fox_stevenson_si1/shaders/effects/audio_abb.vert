...
void main() {
	...
	float t = frac(g_AudioSpectrum32Left[0]);
	...
	v_TexCoordGlitch.y += 0.004 * g_AudioSpectrum32Left[g_Chromatic] + glitchOffset.x+g_DistortionStrength/10*sin(g_AudioSpectrum32Left[1]*rand(vec2(g_Time,g_Time)*2));
	v_TexCoordGlitch.xz += glitchOffset.xy + vec2(0.005, -0.0005) * g_AudioSpectrum32Left[0]*2;
	v_TexCoordGlitch.z -= glitchOffset.z *g_AudioSpectrum32Left[0]/4 ;
	v_TexCoordGlitch.w -= 0.05 * g_AudioSpectrum32Left[0];
	v_TexCoord.x += -g_AudioSpectrum32Left[g_Chromatic]/500 * min(1.0, g_NoiseAlpha);
	v_TexCoord.y -= glitchOffset.z * min(1.0, g_NoiseAlpha)-mul(1,g_AudioSpectrum32Left[g_Chromatic]/50);
}
